...
for n in range(q):
    ...
    for items in temp:
        ...
        count = count + 1
f = open("MUVERA_top-" + str(top_k) + "_equidepth_result.tsv", "w")
f.write(result_string)
f.close()
end_time = time.perf_counter()
elapsed_concatenation_time = end_time - start_time

open(str(dataset_name) + "_top" + str(top_k) + "_equidepth_time.txt", 'w').close()
f = open(str(dataset_name) + "_top" + str(top_k) + "_equidepth_time.txt", "a")
